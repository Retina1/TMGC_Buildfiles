...
@ Check whether to update unit location or not, extensive
ldr   r5, =GetGameClock
bl    GOTO_R5
mov   r1, #0x2                    @ Update location every 2 frames, halves movement speed.
ldr   r5, =_umodsi3
bl    GOTO_R5
lsl   r0, #0x10
mov   r8, r0                      @ Bool indicating whether location should be updated
                                  @ Shared with the bool below
...
mov   r0, r8
add   r0, r1
mov   r8, r0                      @ Bool indicating whether VRAM should be updated
b     MapSpriteLoop
...
Loop:
  ...
  cmp   r6, #0x0                  @ We 'short-circuit' to end, because...
  beq   Return                    @ ...if a unit isn't active, the same goes for its successors
  mov   r3, #0x18
  lsl   r3, #0x4
  cmp   r6, r3
  ...
  ldr   r2, [r3, r1]
  cmp   r2, #0x0
  beq   SkipUnit                  @ If no class data, don't draw
  add   r1, #0x8
  ldrb  r1, [r3, r1]
  mov   r0, #0x4
  and   r1, r0
  cmp   r1, #0x0
  bne   SkipUnit                  @ Unit is dead, don't draw
    ...
    DrawUnit:
    @ Check whether to update unit location or not, lite
    mov   r1, r8
    lsr   r1, #0x10
    cmp   r1, #0x0
    bne   PrepareOAM
